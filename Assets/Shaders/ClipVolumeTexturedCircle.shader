Shader "Custom/ClipVolumeTexturedCircle"
{
	Properties{
...
#include "../MTE/Plugins/MTE/Resources/MTE Common.cginc"
	...
	void surf(Input IN, inout SurfaceOutputStandard o)
	{
		...
		float2 offset;
		offset = IN.worldPos.xz;
		float outOfBounds = max(offset.x, offset.y);
		offset = -IN.worldPos.xz;
		outOfBounds = max(outOfBounds, max(offset.x, offset.y))-1;
